...
def main():
    ...
    if not ollama_available:
        st.warning("⚠️ **Ollama 服务未连接**")
        st.info("请确保：")
        st.info("1. 已安装 Ollama（下载地址：https://ollama.com/download）")
        st.info("2. Ollama 服务正在运行（运行 `ollama serve`）")
        st.info("3. 已下载所需模型（`ollama pull deepseek-r1:7b` 和 `ollama pull nomic-embed-text`）")
        st.stop()
            ...
            if st.button("🔨 构建知识库", type="primary", use_container_width=True):
                if st.session_state.uploaded_files:
                    ...
                    if qa_system is None:
                        st.error("无法创建问答系统，请检查 Ollama 连接")
                        return
    ...
    if prompt := st.chat_input("请输入您的问题..."):
        ...
        with st.chat_message("assistant"):
            ...
            if qa_system is None:
                message_placeholder.markdown("❌ 无法创建问答系统，请检查 Ollama 连接")
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": "无法创建问答系统，请检查 Ollama 连接",
                    "sources": []
                })
                return
            ...
            try:
                ...
                message_placeholder.markdown(f"❌ 问答过程中出错: {str(e)}")
                sources = []
            
            st.session_state.messages.append({
                "role": "assistant",
                "content": full_response if 'full_response' in locals() else "问答过程中出错",
